...
if echo "$SEARCH_RESPONSE" | jq -e '.results' > /dev/null 2>&1; then
    ...
    if [ "$RESULT_COUNT" -gt 0 ]; then
        ...
        echo "  Sample result:"
        echo "$SEARCH_RESPONSE" | jq -r '.results[0].text[:100]' 2>/dev/null || \
        echo "$SEARCH_RESPONSE" | jq -r '.results[0].text' | head -c 100 2>/dev/null || true
        echo "..."
    else
        echo "✗ WARNING: Vector search returned 0 results"
    fi
else
