...
[TIP]
====
*What is a CRD?* A Custom Resource Definition extends the Kubernetes API with new resource types. The `LlamaStackDistribution` CRD teaches OpenShift how to manage Llama Stack servers as first-class Kubernetes objects — meaning you can create, update, and delete Llama Stack instances using the same `oc apply` workflow you use for any other OpenShift resource.
====

Using *Terminal 1*, check if the LlamaStackDistribution CRD is available:
...
The lab materials are stored in a GitHub repository, you can find the URL in the *Resources* section of this page.

Now, git-clone a copy of the project files into your home directory:
...
[TIP]
====
*What does the ConfigMap configure?* The `llama-stack-config` ConfigMap contains a `run.yaml` file that defines which providers (inference engines, vector stores, safety shields) your Llama Stack distribution will use, along with their connection details. This separates configuration from the deployment manifest, making it easy to change providers without redeploying the server.
====

You can also view the llamastack configuration in the ConfigMap via the `oc` command.
...
[TIP]
====
*Key fields in the manifest below:*

* `distribution.name: rh-dev` — selects the Red Hat development distribution, which pre-configures supported providers
* `port: 8321` — the standard Llama Stack server port that all clients connect to
* `VLLM_API_URL` / `VLLM_API_TOKEN` — points Llama Stack at the MaaS endpoint where your models are hosted, so Llama Stack proxies inference requests to vLLM
* `userConfig.configMapName` — links to the ConfigMap you just created, which defines the full provider configuration
====
...
[NOTE]
====
*What just happened?* The Llama Stack Operator saw your `LlamaStackDistribution` custom resource and automatically created a Deployment (which manages the pod) and a ClusterIP Service (which gives it a stable DNS name within the cluster). Your applications will connect to Llama Stack via the service name `llamastack-distribution-vllm-service` on port `8321` — you'll use this URL throughout the remaining lab modules.
====
